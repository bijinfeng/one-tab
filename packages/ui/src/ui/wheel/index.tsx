...
import { type EventTypes, type FullGestureState, useDrag, useWheel } from "@use-gesture/react";
...
import type { PickerValue, WheelProps } from "./type";
	...
	(props) => {
		...
		return (
			<div className={`${classPrefix}-column`}>
				<div className={`${classPrefix}-item-height-measure`} ref={itemHeightMeasureRef} />
				<animated.div ref={rootRef} style={{ translateY: y }} className={`${classPrefix}-column-wheel`} aria-hidden>
					{column.map((item, index) => {
						...
						return (
							<div
								...
								className={cn(`${classPrefix}-column-item`, {
							...
							>
								<div className={`${classPrefix}-column-item-label`}>{renderLabel(item)}</div>
							</div>
						);
					})}
				</animated.div>
