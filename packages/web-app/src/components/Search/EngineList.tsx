...
export function EngineList() {
  const { currentId, defaultList, currentIdList } = useSearchStore()

  const currentEngineList = currentIdList.map(it => defaultList.find(item => item.id === it)!)

  return (
    <div className="engine-box glass-card px-[20px] pt-[20px] pb-[24px]">
      <ul className="wrapper text-[12px] relative grid grid-cols-[repeat(auto-fill,48px)] gap-[20px]">
        {currentEngineList.map(item => (
          <li key={item.id} className="search-drop flex flex-col items-center">
            <div className="group cursor-pointer icon search-drag relative flex h-[48px] w-[48px] items-center justify-center rounded-[12px] bg-color-white bg-opacity-80 transition-colors hover:bg-opacity-100 dark:bg-opacity-[0.06] dark:hover:bg-opacity-20">
              {currentId !== item.id && (
                <section className="hi-close flex items-center justify-center rounded-[50%] bg-color-b4 shadow-close-btn transition-opacity absolute top-[-8px] left-[-8px] h-[20px] w-[20px] cursor-default opacity-0 group-hover:opacity-100">
                  <Close strokeWidth={8} size={10} className="text-color-t4 transition-[transform,color] duration-300" />
                </section>
              )}
